...
Goal:
Backtest whether high-confidence rookie-contract surplus candidates
outperform similar rookie-contract players the model did not flag.
...
#print("\nNot-flagged baseline:", not_flagged_baseline.shape)
#print(not_flagged_baseline["position_final"].value_counts())

#print("\nAll rookie-contract baseline:", baseline_pool.shape)
...
all_rookie_contract_results = run_backtest(
    baseline_pool,
    "all_rookie_contract_baseline"
)
...
baseline_summary = summary[
    summary["group"] == "not_flagged_baseline"
].copy()
...
print("\nLift: model candidates vs not-flagged baseline")
print(
    lift[
        [
            "position_final",
...
candidate_results.to_csv(
    f"{OUTPUT_DIR}/model_candidate_backtest_clean.csv",
    index=False
)

not_flagged_results.to_csv(
    f"{OUTPUT_DIR}/not_flagged_baseline_backtest_clean.csv",
    index=False
)

all_rookie_contract_results.to_csv(
    f"{OUTPUT_DIR}/all_rookie_contract_backtest_clean.csv",
    index=False
)

combined.to_csv(
    f"{OUTPUT_DIR}/combined_backtest_clean.csv",
    index=False
)

summary.to_csv(
    f"{OUTPUT_DIR}/backtest_summary_clean.csv",
    index=False
)

lift.to_csv(
    f"{OUTPUT_DIR}/backtest_lift_model_vs_not_flagged_clean.csv",
    index=False
)

print("\nSaved outputs to:", OUTPUT_DIR)
print("Saved clean player-season file to: outputs_v4/player_value_2021_2025_player_season_clean.csv")
